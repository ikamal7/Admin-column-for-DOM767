...
namespace DOM_COLUMN\Column\Free;

class Called extends \AC\Column {

	public function __construct() {

		// Identifier, pick an unique name. Single word, no spaces. Underscores allowed.
		$this->set_type( 'column-called' );

		// Default column label.
		$this->set_label( __( 'Called', 'ac-called' ) );
	}
	...
	public function get_value( $post_id ) {
		...
		$value = '<span class="'. strtolower($class) .'">'. $value .'</span>';
	...
	public function get_raw_value( $post_id ) {
		...
		$value = get_post_meta( $post_id, '_called', true );
	...
	public function scripts() {
		...
		wp_enqueue_script( 'ac-called',  DOM_AC_JS . "/column.js"  );
		wp_enqueue_style( 'ac-called', DOM_AC_CSS . "/column.css" );
	}
